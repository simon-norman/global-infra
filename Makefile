...
https_certificate_down:
	@cd ./src/aws/eu-west-2/https-certificates && $(DOPPLER_CMD) pulumi down -s $(STACK)
...
environment_zones_down:
	@cd ./src/aws/eu-west-2/environment-hosted-zones && $(DOPPLER_CMD) pulumi down -s $(STACK)
.PHONY: environment_zones_down

master_zone_up:
	@cd ./src/aws/eu-west-2/master-hosted-zone && $(DOPPLER_CMD) pulumi up -s master
.PHONY: master_zone_up

master_zone_down:
	@cd ./src/aws/eu-west-2/master-hosted-zone && $(DOPPLER_CMD) pulumi down -s master
...
ec2_cluster_down:
	@cd ./src/aws/eu-west-2/ec2-cluster && $(DOPPLER_CMD) pulumi down -s $(STACK)
